...
psm_glyco %>% 
  filter(time != "168h") %>% 
  group_by(Gene, Protein.Start, time, replicate) %>% 
  summarise(intensity = sum(intensity)) %>% 
  group_by(Gene, Protein.Start, time) %>% 
  summarise(intensity = mean(intensity)) %>% 
  group_by(Gene, Protein.Start) %>% 
  mutate(scaled_int = scale(log2(intensity))) %>% 
  drop_na() %>% 
...
```{r}
res_ogt_glyco$limma_results %>% filter(hit == "hit") %>% 
  mutate(annotation  = ifelse(Gene  %in% psm_glyco$Gene, "glycoprotein", "other")) %>% 
  filter(annotation == "glycoprotein") %>%  select(Gene, logFC, contrast, direction)
  distinct(Gene)
  group_by(annotation, direction, contrast) %>% 
  count

res_ogt_glyco$limma_results %>%
  mutate(annotation = ifelse(Gene %in% psm_glyco$Gene, "glycoprotein", "other")) %>%
  distinct(Gene, annotation, hit) %>%
  group_by(annotation, hit) %>%
  count()
```
